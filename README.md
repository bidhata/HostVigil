...
| **Credential Checker** | Async default/weak credential audit across 10 protocols (SSH, RDP, SMB, WinRM, FTP, HTTP Basic, MySQL, Postgres, MongoDB, Redis) with password-spray + lockout protection, plus backend selection (`native` or optional `spearspray`) |
...
│  📋  File-Only Logging     Syslog-style UTC timestamps │
...
- **Live Logs** — Real-time SSE log stream with severity filters (ERROR/WARN/INFO/DEBUG), search, and UTC date+time display
...
- 📜 Live Logs page with real-time SSE streaming, severity filters, search, and syslog-style UTC timestamps
...
- **SpearSpray integration** (AD/Kerberos-focused), selectable from the Credentials UI
- SpearSpray is configured from the web UI for each run; config.yaml keeps the native backend as default
- If required SpearSpray fields are missing, the UI and API return an explicit error

Credential backend API endpoints:
- `GET /api/credentials/backends` — available backends + configured default
- `POST /api/credentials/check` — start async check (`backend`: `native` or `spearspray`; UI may send `spearspray_config`)
- `GET /api/credentials/status/<check_id>` — async job status/results
...
hostvigil:
  ...
  credential_spray:
    ...
    native:
      ...
      max_delay: 45.0
      max_threads: 3
    spearspray:                    # UI-only run config when backend: spearspray
...
│   │   ├── spearspray_adapter.py   # Optional SpearSpray backend adapter
...
│       └── test_enhancements.py    # RBAC/scheduling/SIEM/credentials enhancements
...
> `ldap3` enables the AD Discovery module; `scapy` powers raw packet crafting (ARP, SYN, passive sniff). Both are installed by default via `requirements.txt`.
