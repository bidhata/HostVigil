...
from hostvigil.dashboard.terminal_guard import ALLOWED_TERMINAL_COMMANDS, parse_and_validate_terminal_command

# RBAC ordering for authorization decisions
ROLE_LEVELS = {"viewer": 10, "operator": 20, "admin": 30}
SEVERITY_LEVELS = {"info": 10, "low": 20, "medium": 30, "high": 40, "critical": 50}
...
def create_app(config: dict = None):
    ...
    if config:
        ...
        if "db_scheduler_enabled" in config:
            app.config["DB_SCHEDULER_ENABLED"] = bool(config["db_scheduler_enabled"])
    ...
    # Optional enterprise auth/logging components (safe no-op fallback).
    api_key_manager = None
    try:
        from hostvigil.enterprise import APIKeyManager, RequestLogger

        api_key_manager = APIKeyManager(app.config["DB_PATH"])
        api_key_manager.ensure_table()
        RequestLogger(app, app.config["DB_PATH"]).register()
    except Exception:
        api_key_manager = None
    ...
    def _normalize_role(role: str | None) -> str:
        ...
        return role_key if role_key in ROLE_LEVELS else "viewer"

    def _role_allows(role: str, required: str) -> bool:
        return ROLE_LEVELS.get(_normalize_role(role), 0) >= ROLE_LEVELS.get(_normalize_role(required), 999)

    def _permissions_to_set(value: str | None) -> set[str]:
        if not value:
            return {"read"}
    ...
    def _token_allows_write(perms: set[str]) -> bool:
        return "write" in perms or "admin" in perms

    def _load_user_role(username: str | None) -> str:
        if not username:
            return "viewer"
        user = query_db("SELECT role FROM users WHERE username = ?", (username,), one=True)
        return _normalize_role((user or {}).get("role"))
    ...
    _credential_jobs: dict[int, dict] = {}
        ...
        if request.path.startswith("/api/"):
            ...
            if token:
                if api_key_manager is None:
                    return jsonify({"error": "API token support unavailable"}), 503
                key_info = api_key_manager.validate_key(token)
                if not key_info:
                    return jsonify({"error": "Invalid API token"}), 401
                perms = _permissions_to_set(key_info.get("permissions"))
                g.auth_type = "api_token"
                g.api_key_id = key_info.get("id")
                g.username = key_info.get("created_by") or "api-token"
                g.permissions = perms
                g.role = "admin" if "admin" in perms else ("operator" if "write" in perms else "viewer")

                if any(request.path.startswith(prefix) for prefix in ADMIN_API_PATH_PREFIXES) and "admin" not in perms:
                    return jsonify({"error": "Admin token required"}), 403
                if _is_write_request() and not _token_allows_write(perms):
            ...
            role = _normalize_role(session.get("role") or _load_user_role(username))
            session["role"] = role
            g.auth_type = "session"
            g.username = username
            g.role = role

            if any(request.path.startswith(prefix) for prefix in ADMIN_API_PATH_PREFIXES) and not _role_allows(
                role, "admin"
            ):
                return jsonify({"error": "Admin role required"}), 403
            if _is_write_request() and not _role_allows(role, "operator"):
                return jsonify({"error": "Operator or admin role required"}), 403
        return None
    ...
    def _emit_siem_event(event_type: str, severity: str = "info", payload: dict | None = None) -> None:
        """Persist and optionally forward SIEM event records."""
        event_payload = payload or {}
        normalized_severity = str(severity).lower()
        if normalized_severity not in SEVERITY_LEVELS:
            normalized_severity = "info"

        serialized = json.dumps(event_payload, default=str, sort_keys=True)
        with get_db() as conn:
            conn.execute(
                "INSERT INTO siem_events (event_type, severity, payload, created_at) VALUES (?, ?, ?, ?)",
                (event_type, normalized_severity, serialized, _now_iso()),
            )
            conn.commit()

        # Optional JSONL sink for file-based SIEM ingestion.
        siem_jsonl_path = (os.environ.get("HOSTVIGIL_SIEM_JSONL") or "").strip()
        if siem_jsonl_path:
            sink_path = Path(siem_jsonl_path)
            ...
            with sink_path.open("a", encoding="utf-8") as f:
                f.write(json.dumps(sink_record, default=str, sort_keys=True) + "\n")

        # Optional webhook sink for central SIEM collector.
        siem_webhook = (os.environ.get("HOSTVIGIL_SIEM_WEBHOOK") or "").strip()
        if siem_webhook:
            import urllib.request

            body = json.dumps(
                {"event_type": event_type, "severity": normalized_severity, "payload": event_payload, "ts": _now_iso()},
                default=str,
            ).encode("utf-8")
            req = urllib.request.Request(
                siem_webhook,
                data=body,
                headers={"Content-Type": "application/json", "User-Agent": "HostVigil-SIEM/1"},
                method="POST",
            )
    ...
    def _severity_meets(severity: str, minimum: str) -> bool:
        return SEVERITY_LEVELS.get(str(severity).lower(), 0) >= SEVERITY_LEVELS.get(str(minimum).lower(), 0)

    def _dispatch_alert(
        event_type: str, title: str, message: str, severity: str = "info", extra: dict | None = None
    ) -> dict:
        """Dispatch alerts to enabled webhooks with severity filtering and dedup."""
        severity_norm = str(severity).lower()
        if severity_norm not in SEVERITY_LEVELS:
            severity_norm = "info"

        rows = query_db("SELECT * FROM webhooks WHERE COALESCE(enabled, 1) = 1 ORDER BY created_at DESC LIMIT 500")
        sent = 0
        skipped = 0
        for row in rows:
            min_sev = (row.get("min_severity") or "info").lower()
            if not _severity_meets(severity_norm, min_sev):
                skipped += 1
                continue
            events_cfg = str(row.get("events") or "all").lower()
            if events_cfg != "all" and event_type not in {e.strip() for e in events_cfg.split(",") if e.strip()}:
                skipped += 1
                continue

            dedup_seconds = int(row.get("dedup_seconds") or 60)
            dedup_raw = f"{row.get('id')}|{event_type}|{title}|{message[:120]}|{severity_norm}"
            dedup_key = hashlib.sha256(dedup_raw.encode("utf-8")).hexdigest()
            with get_db() as conn:
                existing = conn.execute(
                    "SELECT last_sent_at FROM alert_dedup WHERE dedup_key = ?", (dedup_key,)
                ).fetchone()
                if existing and dedup_seconds > 0:
                    age_q = conn.execute(
                        "SELECT CAST((julianday('now') - julianday(?)) * 86400 AS INTEGER)", (existing["last_sent_at"],)
                    ).fetchone()
                    age_seconds = int(age_q[0] or 0) if age_q else 0
            ...
            alerter = WebhookAlerter(
                {
                    "enabled": True,
                    "urls": [row["url"]],
                    "notify_on": [event_type],
                    "rate_limit": 0,
                    "include_details": True,
                },
                app.config["DB_PATH"],
            )
            alerter.notify(event_type, title, message, severity_norm, extra=extra or {})
            with get_db() as conn:
                conn.execute(
                    """
                    INSERT INTO alert_dedup (dedup_key, event_type, severity, last_sent_at)
                    VALUES (?, ?, ?, ?)
                    ...
                    """,
                    (dedup_key, event_type, severity_norm, _now_iso()),
                )
                conn.commit()
            sent += 1
        ...
        _emit_siem_event(
            "alert_dispatched",
            severity_norm,
            {"event_type": event_type, "sent": sent, "skipped": skipped, "title": title},
        )
        return {"sent": sent, "skipped": skipped}

    def _get_credential_spray_config() -> dict:
        defaults = {
            "backend": "native",
            "enabled": True,
        ...
        def _deep_merge_cfg(base: dict, override: dict) -> dict:
            ...
            for k, v in override.items():
                if k in result and isinstance(result[k], dict) and isinstance(v, dict):
                    result[k] = _deep_merge_cfg(result[k], v)
                else:
                    result[k] = v
            return result
        ...
        try:
            if app.orchestrator is not None:
                cfg = app.orchestrator.config.hostvigil.get("credential_spray", {})
                if isinstance(cfg, dict):
                    return _deep_merge_cfg(defaults, cfg)
        except Exception:
            pass
        try:
            from hostvigil.config import get_config

            cfg_obj = get_config("config.yaml")
            cfg = cfg_obj.hostvigil.get("credential_spray", {})
            if isinstance(cfg, dict):
                return _deep_merge_cfg(defaults, cfg)
        except Exception:
            pass
    ...
    def _is_spearspray_configured(cfg: dict) -> bool:
        spearspray_cfg = (cfg or {}).get("spearspray", {})
        required = ("domain", "username", "password", "domain_controller")
        return all(str(spearspray_cfg.get(key, "")).strip() for key in required)

    def _normalize_spearspray_runtime_config(raw_cfg: dict | None) -> dict:
        raw_cfg = raw_cfg if isinstance(raw_cfg, dict) else {}
        normalized = {
            "binary_path": str(raw_cfg.get("binary_path", "")).strip(),
            "domain": str(raw_cfg.get("domain", "")).strip(),
            "username": str(raw_cfg.get("username", "")).strip(),
            "password": str(raw_cfg.get("password", "")).strip(),
            "domain_controller": str(raw_cfg.get("domain_controller", "")).strip(),
            "use_ssl": bool(raw_cfg.get("use_ssl", False)),
            "threads": raw_cfg.get("threads", 5),
            "jitter": str(raw_cfg.get("jitter", "")).strip(),
            "max_rps": raw_cfg.get("max_rps", None),
            "threshold": raw_cfg.get("threshold", 2),
            "extra": str(raw_cfg.get("extra", "")).strip(),
            "separator": str(raw_cfg.get("separator", "")).strip(),
            "suffix": str(raw_cfg.get("suffix", "")).strip(),
            "query": str(raw_cfg.get("query", "")).strip(),
            "patterns_file": str(raw_cfg.get("patterns_file", "")).strip(),
        }

        for key in ("threads", "threshold"):
            value = normalized[key]
            if value in (None, ""):
                normalized[key] = 5 if key == "threads" else 2
            else:
                normalized[key] = int(value)

        value = normalized["max_rps"]
        if value in (None, ""):
            normalized["max_rps"] = None
        else:
            normalized["max_rps"] = int(value)

        return normalized

    def _merge_spearspray_runtime_config(base_cfg: dict, ui_cfg: dict | None) -> dict:
        merged = dict(base_cfg or {})
        spearspray_cfg = dict(merged.get("spearspray", {})) if isinstance(merged.get("spearspray", {}), dict) else {}
        if isinstance(ui_cfg, dict):
            for key in ("domain", "username", "password", "domain_controller"):
                spearspray_cfg[key] = ""
            normalized = _normalize_spearspray_runtime_config(ui_cfg)
            for key, value in normalized.items():
                if key in {"use_ssl", "threads", "threshold", "max_rps"}:
                    spearspray_cfg[key] = value
                elif value != "":
                    spearspray_cfg[key] = value
        merged["spearspray"] = spearspray_cfg
        return merged

    def _get_bounded_limit(name: str, default: int, maximum: int) -> int:
        """Read a request limit parameter bounded to a safe range."""
        try:
    ...
    def _cached_json_response(cache_namespace: str, ttl_seconds: int, producer):
        """Return cached JSON payload for heavy read endpoints."""
        key = f"{cache_namespace}?{request.query_string.decode('utf-8', errors='ignore')}"
        now = time.time()
        ...
        payload = producer()
        with _api_response_cache_lock:
            _api_response_cache[key] = {"payload": payload, "expires_at": now + ttl_seconds}
    ...
    def login():
        ...
        if request.method == "POST":
            ...
            if user and check_password_hash(user["password_hash"], password):
                ...
                session["role"] = _normalize_role(user.get("role"))
    ...
    def api_login():
        """Programmatic login endpoint for session-based API access."""
        from werkzeug.security import check_password_hash
    ...
    @login_required
    def api_logs_stream():
        """SSE endpoint for streaming live log entries."""
    ...
    @app.route("/api/vulnerabilities/<int:vuln_id>/lifecycle", methods=["GET", "POST"])
    def api_vulnerability_lifecycle(vuln_id):
        """Get/update lifecycle fields for a vulnerability."""
        vuln = query_db(
            """
            SELECT id, status, owner, due_date, accepted_risk, updated_at
        ...
        if request.method == "GET":
            ...
            return jsonify({"lifecycle": vuln, "comments": comments})

        if not request.is_json:
            return jsonify({"error": "JSON required"}), 400
        allowed_status = {"open", "in_progress", "resolved", "accepted_risk", "false_positive"}
        status_value = request.json.get("status")
        owner = request.json.get("owner")
        due_date = request.json.get("due_date")
        accepted_risk = request.json.get("accepted_risk")

        if status_value is not None and str(status_value).lower() not in allowed_status:
        ...
        if status_value is not None:
            ...
            params.append(str(status_value).lower())
        if owner is not None:
            updates.append("owner = ?")
            params.append(str(owner).strip() or None)
        if due_date is not None:
            updates.append("due_date = ?")
            params.append(str(due_date).strip() or None)
        if accepted_risk is not None:
            updates.append("accepted_risk = ?")
            params.append(1 if bool(accepted_risk) else 0)

        if not updates:
            return jsonify({"error": "No lifecycle fields provided"}), 400

        updates.append("updated_at = ?")
        params.append(_now_iso())
        params.append(vuln_id)
        with get_db() as conn:
            conn.execute(f"UPDATE vulnerabilities SET {', '.join(updates)} WHERE id = ?", tuple(params))
            conn.commit()

        updated = query_db(
            "SELECT id, status, owner, due_date, accepted_risk, updated_at FROM vulnerabilities WHERE id = ?",
            (vuln_id,),
            one=True,
        )
        _emit_siem_event(
            "vulnerability_lifecycle_updated",
            "high",
            {
                "vulnerability_id": vuln_id,
                "updated_by": g.username or session.get("username"),
                "status": updated.get("status"),
                "owner": updated.get("owner"),
                "accepted_risk": updated.get("accepted_risk"),
            },
        )
        return jsonify({"status": "updated", "lifecycle": updated})

    @app.route("/api/vulnerabilities/<int:vuln_id>/comments", methods=["POST"])
    def api_vulnerability_comment(vuln_id):
        """Add a workflow comment to a vulnerability."""
        if not request.is_json:
            return jsonify({"error": "JSON required"}), 400
        comment = (request.json.get("comment") or "").strip()
        if not comment:
            return jsonify({"error": "comment is required"}), 400
        if not query_db("SELECT id FROM vulnerabilities WHERE id = ?", (vuln_id,), one=True):
            return jsonify({"error": "Vulnerability not found"}), 404

        with get_db() as conn:
            conn.execute(
                """
                INSERT INTO vulnerability_comments (vulnerability_id, author, comment, created_at)
                VALUES (?, ?, ?, ?)
                """,
                (vuln_id, g.username or session.get("username") or "unknown", comment, _now_iso()),
            )
            conn.commit()
        return jsonify({"status": "added", "vulnerability_id": vuln_id})
    ...
    def notes():
        ...
        notes_limit = _get_bounded_limit("limit", default=500, maximum=2000)
        all_notes = query_db("SELECT * FROM notes ORDER BY created_at DESC LIMIT ?", (notes_limit,))
    ...
    def _execute_scheduled_scan(schedule_id: int, schedule: dict | None = None) -> dict:
        if schedule is None:
        ...
        policy = None
        if schedule.get("policy_id"):
            policy = query_db("SELECT * FROM schedule_policies WHERE id = ?", (schedule["policy_id"],), one=True)

        scan_type = (policy or {}).get("scan_type") or schedule.get("scan_type") or "full"
        if scan_type == "discovery":
            result = app.orchestrator.run_discovery()
        elif scan_type == "scan":
            result = app.orchestrator.run_scan()
        elif scan_type == "analysis":
            result = app.orchestrator.run_analysis()
        elif scan_type == "nuclei":
            result = app.orchestrator.run_nuclei()
        else:
            result = app.orchestrator.run_once()

        with get_db() as conn:
            conn.execute("UPDATE schedules SET last_run_at = ? WHERE id = ?", (_now_iso(), schedule_id))
            conn.commit()

        vulns_found = 0
        if isinstance(result, dict):
            vulns_found = int(result.get("vulnerabilities_found") or 0)
            if not vulns_found and isinstance(result.get("phases"), dict):
                vulns_found = int((result["phases"].get("nuclei") or {}).get("vulnerabilities_found") or 0)
        if vulns_found > 0:
            _dispatch_alert(
                event_type="critical_vuln",
                title="Scheduled scan detected vulnerabilities",
                message=f"Schedule {schedule_id} completed with {vulns_found} vulnerability findings.",
                severity="high",
                extra={"schedule_id": schedule_id, "scan_type": scan_type, "vulnerabilities_found": vulns_found},
            )

        _emit_siem_event(
            "scheduled_scan_executed",
            "medium",
            {"schedule_id": schedule_id, "scan_type": scan_type, "policy_id": schedule.get("policy_id")},
        )
        return {"schedule_id": schedule_id, "scan_type": scan_type, "result": result}

    @app.route("/api/schedule", methods=["GET"])
    def api_get_schedules():
        """API: Get all scan schedules."""
        limit = _get_bounded_limit("limit", default=200, maximum=1000)
        schedules = query_db(
            """
            SELECT s.*, sp.name AS policy_name
            FROM schedules s
            LEFT JOIN schedule_policies sp ON sp.id = s.policy_id
    ...
    def api_add_schedule():
        ...
        if policy_id is not None:
            policy = query_db("SELECT id FROM schedule_policies WHERE id = ?", (policy_id,), one=True)
            if not policy:
                return jsonify({"error": "policy_id not found"}), 404
        now = datetime.now(timezone.utc).isoformat()
        with get_db() as conn:
            conn.execute(
                """
                INSERT INTO schedules (scan_type, cron_expr, enabled, created_at, policy_id, next_run_hint)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (scan_type, cron_expr, 1 if enabled else 0, now, policy_id, "cron"),
            )
            conn.commit()
        _emit_siem_event(
            "schedule_created",
            "info",
            {
                ...
                "cron_expr": cron_expr,
                "policy_id": policy_id,
                "created_by": g.username or session.get("username"),
            },
        )
        return jsonify({"status": "created", "scan_type": scan_type, "cron_expr": cron_expr, "policy_id": policy_id})
    ...
    @app.route("/api/schedule/policies", methods=["GET"])
    def api_get_schedule_policies():
        limit = _get_bounded_limit("limit", default=200, maximum=2000)
        policies = query_db("SELECT * FROM schedule_policies ORDER BY created_at DESC LIMIT ?", (limit,))
        return jsonify({"policies": policies})

    @app.route("/api/schedule/policies", methods=["POST"])
    def api_add_schedule_policy():
        if not request.is_json:
            return jsonify({"error": "JSON required"}), 400
        name = (request.json.get("name") or "").strip()
        scan_type = (request.json.get("scan_type") or "full").strip()
        port_profile = (request.json.get("port_profile") or "standard").strip()
        nuclei_enabled = bool(request.json.get("nuclei_enabled", True))
        ...
        with get_db() as conn:
            try:
                conn.execute(
                    """
                    INSERT INTO schedule_policies (name, scan_type, port_profile, nuclei_enabled, notes, created_at)
                    VALUES (?, ?, ?, ?, ?, ?)
                    """,
                    (name, scan_type, port_profile, 1 if nuclei_enabled else 0, notes, _now_iso()),
                )
                conn.commit()
            except sqlite3.IntegrityError:
                return jsonify({"error": "policy name already exists"}), 409
        _emit_siem_event(
            "schedule_policy_created",
            "info",
            {
                "name": name,
                "scan_type": scan_type,
    ...
    @app.route("/api/schedule/policies/<int:policy_id>", methods=["DELETE"])
    def api_delete_schedule_policy(policy_id):
        with get_db() as conn:
            in_use = conn.execute("SELECT 1 FROM schedules WHERE policy_id = ? LIMIT 1", (policy_id,)).fetchone()
            if in_use:
                return jsonify({"error": "Policy is in use by one or more schedules"}), 409
            cur = conn.execute("DELETE FROM schedule_policies WHERE id = ?", (policy_id,))
            conn.commit()
            if cur.rowcount == 0:
                return jsonify({"error": "Policy not found"}), 404
        _emit_siem_event("schedule_policy_deleted", "medium", {"policy_id": policy_id})
        return jsonify({"status": "deleted", "policy_id": policy_id})

    @app.route("/api/schedule/<int:schedule_id>/run", methods=["POST"])
    def api_run_schedule(schedule_id):
        schedule = query_db("SELECT * FROM schedules WHERE id = ?", (schedule_id,), one=True)
        if not schedule:
            return jsonify({"error": "Schedule not found"}), 404
        try:
            execution = _execute_scheduled_scan(schedule_id, schedule)
        except Exception as e:
            return jsonify({"error": str(e)}), 500
        return jsonify({"status": "executed", **execution})

    # -------------------------------------------------------------------
    # RBAC and API token management
    # -------------------------------------------------------------------
    ...
    def api_list_tokens():
        if api_key_manager is None:
            return jsonify({"error": "API token support unavailable"}), 503
        return jsonify({"tokens": api_key_manager.list_keys()})
    ...
    def api_create_token():
        ...
        if not request.is_json:
            return jsonify({"error": "JSON required"}), 400
        name = (request.json.get("name") or "").strip()
        permissions = (request.json.get("permissions") or "read").strip().lower()
        expires_days = request.json.get("expires_days")
        if not name:
            return jsonify({"error": "name is required"}), 400
        allowed_perms = {"read", "write", "admin"}
        perm_set = _permissions_to_set(permissions)
        if not perm_set.issubset(allowed_perms):
            return jsonify({"error": "permissions must be read, write, admin, or comma-separated combinations"}), 400
        raw_token = api_key_manager.create_key(
            name=name,
            created_by=(g.username or session.get("username") or "admin"),
            expires_days=expires_days,
            permissions=",".join(sorted(perm_set)),
        )
        _emit_siem_event(
            "api_token_created",
            "medium",
            {"name": name, "created_by": g.username or session.get("username"), "permissions": sorted(perm_set)},
        )
        return jsonify({"status": "created", "token": raw_token, "name": name, "permissions": sorted(perm_set)}), 201

    @app.route("/api/auth/tokens/<int:key_id>", methods=["DELETE"])
    def api_revoke_token(key_id):
        if api_key_manager is None:
            return jsonify({"error": "API token support unavailable"}), 503
        ok = api_key_manager.revoke_key(key_id)
        if not ok:
            return jsonify({"error": "Token revoke failed"}), 500
        _emit_siem_event(
            "api_token_revoked", "medium", {"key_id": key_id, "revoked_by": g.username or session.get("username")}
        )
        return jsonify({"status": "revoked", "key_id": key_id})

    @app.route("/api/users/roles", methods=["GET"])
    def api_list_user_roles():
        rows = query_db("SELECT id, username, role, created_at FROM users ORDER BY username ASC")
        return jsonify({"users": rows})

    @app.route("/api/users/roles/<int:user_id>", methods=["POST"])
    def api_update_user_role(user_id):
        if not request.is_json:
            return jsonify({"error": "JSON required"}), 400
        role = _normalize_role(request.json.get("role"))
        ...
        _emit_siem_event(
            "user_role_updated",
            "high",
            {"user_id": user_id, "role": role, "updated_by": g.username or session.get("username")},
        )
    ...
    def api_get_profiles():
        ...
        limit = _get_bounded_limit("limit", default=200, maximum=1000)
        profiles = query_db("SELECT * FROM profiles ORDER BY created_at DESC LIMIT ?", (limit,))
    ...
    def api_get_webhooks():
        ...
        limit = _get_bounded_limit("limit", default=200, maximum=1000)
        webhooks = query_db("SELECT * FROM webhooks ORDER BY created_at DESC LIMIT ?", (limit,))
    ...
    def api_add_webhook():
        ...
        min_severity = (request.json.get("min_severity") or "info").strip().lower()
        dedup_seconds = int(request.json.get("dedup_seconds", 60))
        enabled = bool(request.json.get("enabled", True))
        if not name or not url:
            return jsonify({"error": "name and url are required"}), 400
        if min_severity not in SEVERITY_LEVELS:
            return jsonify({"error": "min_severity must be one of info, low, medium, high, critical"}), 400
        if dedup_seconds < 0:
            return jsonify({"error": "dedup_seconds must be >= 0"}), 400
        now = datetime.now(timezone.utc).isoformat()
        with get_db() as conn:
            conn.execute(
                """
                INSERT INTO webhooks (name, url, events, min_severity, dedup_seconds, enabled, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?)
                """,
                (name, url, events, min_severity, dedup_seconds, 1 if enabled else 0, now),
            )
            conn.commit()
        _emit_siem_event(
            "webhook_added",
    ...
    def _get_stats() -> dict:
        ...
        recent_scans = query_db(
            "SELECT * FROM scans WHERE start_time >= datetime('now', '-7 days') ORDER BY start_time DESC LIMIT 50"
        )
        try:
            credential_hits = query_db(
                "SELECT COUNT(*) as count FROM credential_results WHERE success = 1",
                one=True,
            )
        except Exception:
        ...
        result = {
            ...
            "credential_hits": credential_hits["count"] if credential_hits else 0,
            "recent_scans": recent_scans,
        }
    ...
    def api_hosts_by_tag(tag):
        ...
        def _build():
            ...
            return {"tag": tag, "hosts": hosts, "count": len(hosts)}

        return _cached_json_response("hosts_by_tag", 8, _build)
    ...
    def api_nuclei_rules():
        ...
        limit = _get_bounded_limit("limit", default=200, maximum=1000)
        rules = query_db("SELECT * FROM nuclei_rules ORDER BY created_at DESC LIMIT ?", (limit,))
    ...
    def api_personas():
        ...
        if request.method == "POST":
            ...
            return jsonify({"status": "created"})
        limit = _get_bounded_limit("limit", default=200, maximum=1000)
        personas = query_db("SELECT * FROM scan_personas ORDER BY created_at DESC LIMIT ?", (limit,))
    ...
    def api_credentials():
        ...
        def _build():
            try:
                limit = _get_bounded_limit("limit", default=1000, maximum=5000)
                return query_db(
                    """
        ...
        return _cached_json_response("credentials", 8, _build)
    ...
    def api_honeytokens():
        ...
        return _cached_json_response("honeytokens", 8, _build)
    ...
    def api_baselines():
        """Create or list baseline snapshots for drift detection."""
        if request.method == "GET":
            limit = _get_bounded_limit("limit", default=200, maximum=2000)
            rows = query_db("SELECT * FROM baseline_snapshots ORDER BY created_at DESC LIMIT ?", (limit,))
            return jsonify({"baselines": rows})
        ...
        hosts = query_db("SELECT ip, hostname FROM hosts WHERE is_active = 1")
        ports = query_db(
            """
            SELECT h.ip, p.port, p.protocol, p.service
            FROM ports p
            JOIN hosts h ON h.id = p.host_id
            WHERE p.is_active = 1
            """
        ...
        vulns = query_db(
            """
            SELECT h.ip, COALESCE(p.port, 0) AS port, v.template_id, v.name, v.severity
            FROM vulnerabilities v
            JOIN hosts h ON h.id = v.host_id
            LEFT JOIN ports p ON p.id = v.port_id
            """
        ...
        with get_db() as conn:
            cur = conn.execute(
                "INSERT INTO baseline_snapshots (name, created_by, created_at, notes) VALUES (?, ?, ?, ?)",
                (name, g.username or session.get("username"), _now_iso(), notes),
            )
            snapshot_id = cur.lastrowid

            for row in hosts:
                item_key = f"host:{row.get('ip')}"
                conn.execute(
                    "INSERT INTO baseline_snapshot_items (snapshot_id, item_type, item_key, item_payload) VALUES (?, 'host', ?, ?)",
                    (snapshot_id, item_key, json.dumps(row, default=str)),
                )
            for row in ports:
                item_key = f"port:{row.get('ip')}:{row.get('port')}:{row.get('protocol')}"
                conn.execute(
                    "INSERT INTO baseline_snapshot_items (snapshot_id, item_type, item_key, item_payload) VALUES (?, 'port', ?, ?)",
                    (snapshot_id, item_key, json.dumps(row, default=str)),
                )
            for row in vulns:
                item_key = (
                    f"vuln:{row.get('ip')}:{row.get('port')}:{row.get('template_id') or ''}:{row.get('name') or ''}"
                )
                conn.execute(
                    "INSERT INTO baseline_snapshot_items (snapshot_id, item_type, item_key, item_payload) VALUES (?, 'vuln', ?, ?)",
                    (snapshot_id, item_key, json.dumps(row, default=str)),
                )
            conn.commit()
        ...
        return jsonify({"status": "created", "snapshot_id": snapshot_id, "name": name}), 201

    @app.route("/api/baselines/<int:snapshot_id>/drift")
    def api_baseline_drift(snapshot_id):
        """Compare current state against a saved baseline snapshot."""
        snapshot = query_db("SELECT * FROM baseline_snapshots WHERE id = ?", (snapshot_id,), one=True)
        if not snapshot:
            return jsonify({"error": "Baseline snapshot not found"}), 404

        baseline_items = query_db(
            "SELECT item_type, item_key FROM baseline_snapshot_items WHERE snapshot_id = ?",
            (snapshot_id,),
        )
        baseline_by_type = {"host": set(), "port": set(), "vuln": set()}
        for item in baseline_items:
            if item["item_type"] in baseline_by_type:
                baseline_by_type[item["item_type"]].add(item["item_key"])

        current_hosts = {f"host:{r['ip']}" for r in query_db("SELECT ip FROM hosts WHERE is_active = 1")}
        current_ports = {
            f"port:{r['ip']}:{r['port']}:{r['protocol']}"
            for r in query_db(
                """
        ...
        current_vulns = {
            f"vuln:{r['ip']}:{r['port']}:{r['template_id'] or ''}:{r['name'] or ''}"
            for r in query_db(
                """
                SELECT h.ip, COALESCE(p.port, 0) AS port, v.template_id, v.name
                FROM vulnerabilities v
                JOIN hosts h ON h.id = v.host_id
                LEFT JOIN ports p ON p.id = v.port_id
                """
        ...
        added_hosts = sorted(current_hosts - baseline_by_type["host"])
        removed_hosts = sorted(baseline_by_type["host"] - current_hosts)
        added_ports = sorted(current_ports - baseline_by_type["port"])
        removed_ports = sorted(baseline_by_type["port"] - current_ports)
        added_vulns = sorted(current_vulns - baseline_by_type["vuln"])
        removed_vulns = sorted(baseline_by_type["vuln"] - current_vulns)
        ...
        drift = {
            ...
            "summary": {
                ...
                "added_ports": len(added_ports),
                "removed_ports": len(removed_ports),
                "added_vulns": len(added_vulns),
                "removed_vulns": len(removed_vulns),
            },
            "added_hosts": added_hosts[:1000],
            "removed_hosts": removed_hosts[:1000],
            "added_ports": added_ports[:1000],
            "removed_ports": removed_ports[:1000],
            "added_vulns": added_vulns[:1000],
            "removed_vulns": removed_vulns[:1000],
        }
        _emit_siem_event("baseline_drift_calculated", "medium", {"snapshot_id": snapshot_id, **drift["summary"]})
        return jsonify(drift)

    @app.route("/api/alerts/history")
    def api_alerts_history():
        limit = _get_bounded_limit("limit", default=200, maximum=2000)
        rows = query_db("SELECT * FROM alert_history ORDER BY sent_at DESC LIMIT ?", (limit,))
    ...
    def api_alerts_send():
        if not request.is_json:
            return jsonify({"error": "JSON required"}), 400
        event_type = (request.json.get("event_type") or "").strip().lower()
        title = (request.json.get("title") or "").strip()
        message = (request.json.get("message") or "").strip()
        severity = (request.json.get("severity") or "info").strip().lower()
        extra = request.json.get("extra") or {}
        if not event_type or not title or not message:
            return jsonify({"error": "event_type, title, and message are required"}), 400
        if severity not in SEVERITY_LEVELS:
            return jsonify({"error": "invalid severity"}), 400
        result = _dispatch_alert(event_type=event_type, title=title, message=message, severity=severity, extra=extra)
        return jsonify({"status": "sent", **result})

    @app.route("/api/siem/events")
    def api_siem_events():
        limit = _get_bounded_limit("limit", default=500, maximum=5000)
        severity = (request.args.get("severity") or "").strip().lower()
        if severity:
            rows = query_db(
                "SELECT * FROM siem_events WHERE severity = ? ORDER BY created_at DESC LIMIT ?",
                (severity, limit),
            )
        else:
            rows = query_db("SELECT * FROM siem_events ORDER BY created_at DESC LIMIT ?", (limit,))
    ...
    def api_kill_chain():
        ...
        if request.method == "POST":
            ...
            with get_db() as conn:
                ...
                conn.commit()
            return jsonify({"status": "added"})
        limit = _get_bounded_limit("limit", default=500, maximum=3000)
        chain = query_db("SELECT * FROM kill_chain ORDER BY step_order ASC LIMIT ?", (limit,))
    ...
    def api_terminal():
        ...
        try:
            parts = parse_and_validate_terminal_command(cmd)
        except ValueError as e:
            return jsonify({"error": str(e)}), 400
        except PermissionError as e:
            return jsonify({"error": str(e), "allowed": list(ALLOWED_TERMINAL_COMMANDS)}), 403
        except Exception as e:
            return jsonify({"error": f"Command validation failed: {e}"}), 400

        if not parts:
            return jsonify({"error": "No command provided"}), 400

        run_py_path = (Path(app.config["DB_PATH"]).parent.parent / "run.py").resolve()
        if not run_py_path.exists():
            return jsonify({"error": f"Missing CLI entrypoint: {run_py_path}"}), 500

        try:
            # Use list form and current interpreter to prevent shell injection and venv drift.
            result = subprocess.run(
                [sys.executable, str(run_py_path)] + parts,
                capture_output=True,
                text=True,
                timeout=30,
    ...
    def _start_db_scheduler() -> None:
        """Start cron scheduler for enabled DB-backed scan schedules."""
        if not app.config.get("DB_SCHEDULER_ENABLED", True):
            return
        try:
            from hostvigil.scheduler import CronScheduler
        except Exception:
            return

        scheduler = CronScheduler(config={})
        schedules = query_db(
            "SELECT id, cron_expr FROM schedules WHERE COALESCE(enabled, 1) = 1 ORDER BY created_at DESC LIMIT 1000"
        )
        for row in schedules:
            schedule_id = int(row["id"])
            cron_expr = (row.get("cron_expr") or "").strip()
            if not cron_expr:
                continue

            def _run_with_context(sid=schedule_id):
                with app.app_context():
                    try:
                        _execute_scheduled_scan(sid)
                    except Exception as e:
                        _emit_siem_event("scheduled_scan_failed", "high", {"schedule_id": sid, "error": str(e)})

            scheduler.add_cron_job(f"db-schedule-{schedule_id}", _run_with_context, cron_expr, jitter=30)

        scheduler.start()
        app._db_scheduler = scheduler
    ...
    @app.route("/api/credentials/backends")
    @api_login_required
    def api_credentials_backends():
        cfg = _get_credential_spray_config()
        default_backend = str(cfg.get("backend", "native")).strip().lower()
        if default_backend not in {"native", "spearspray"}:
            default_backend = "native"
        return jsonify(
            {
                "default_backend": default_backend,
                "available": ["native", "spearspray"],
                "enabled": bool(cfg.get("enabled", True)),
                "spearspray_configured": _is_spearspray_configured(cfg),
            }
        )
    ...
    def api_credentials_check():
        """Run credential check on services."""
        if not request.is_json:
            return jsonify({"error": "JSON required"}), 400

        data = request.json or {}
        scope = str(data.get("scope") or "all").strip().lower()
        services = data.get("services") or ["all"]
        if not isinstance(services, list):
            return jsonify({"error": "services must be a list"}), 400
        requested_backend = str(data.get("backend") or "").strip().lower()
        ui_spearspray_cfg = data.get("spearspray_config")

        cfg = _get_credential_spray_config()
        if not bool(cfg.get("enabled", True)):
            return jsonify({"error": "Credential spray is disabled in configuration"}), 403

        backend = requested_backend or str(cfg.get("backend", "native")).strip().lower()
        if backend not in {"native", "spearspray"}:
            return jsonify({"error": "backend must be 'native' or 'spearspray'"}), 400
        if backend == "spearspray":
            try:
                cfg = _merge_spearspray_runtime_config(
                    cfg, ui_spearspray_cfg if isinstance(ui_spearspray_cfg, dict) else None
                )
            except (TypeError, ValueError) as exc:
                return jsonify({"error": f"Invalid SpearSpray settings: {exc}"}), 400
            if not _is_spearspray_configured(cfg):
                if isinstance(ui_spearspray_cfg, dict):
                    return jsonify({"error": "SpearSpray form is missing required fields"}), 400
                return jsonify({"error": "SpearSpray backend is not fully configured in config.yaml"}), 400

        check_id = int(uuid.uuid4().int & 0x7FFFFFFF)
        now = _now_iso()
        with _credential_jobs_lock:
            _credential_jobs[check_id] = {
                "status": "queued",
                "progress": 0,
                "successes": 0,
                "results": [],
        ...
        def _runner():
            try:
                with _credential_jobs_lock:
                    _credential_jobs[check_id]["status"] = "running"
                    _credential_jobs[check_id]["progress"] = 10

                if backend == "spearspray":
                    from hostvigil.scanner.spearspray_adapter import SpearSprayAdapter

                    adapter = SpearSprayAdapter(cfg, app.config["DB_PATH"])
                    ...
                    native_cfg = cfg.get("native", {}) if isinstance(cfg.get("native"), dict) else {}
                    spray = StealthCredentialSpray(native_cfg, app.config["DB_PATH"])
                    results = spray.spray_all(services=services)
                    for row in results:
                        row.setdefault("method", "HostVigil Native Spray")
                    meta = {}

                success_count = len([r for r in results if r.get("success")])
                with _credential_jobs_lock:
                    _credential_jobs[check_id]["status"] = "completed"
                    _credential_jobs[check_id]["progress"] = 100
                    _credential_jobs[check_id]["successes"] = success_count
                    _credential_jobs[check_id]["results"] = results
                    _credential_jobs[check_id]["meta"] = meta
                    _credential_jobs[check_id]["finished_at"] = _now_iso()
                ...
                with _credential_jobs_lock:
                    _credential_jobs[check_id]["status"] = "failed"
                    _credential_jobs[check_id]["progress"] = 100
                    _credential_jobs[check_id]["error"] = str(exc)
                    _credential_jobs[check_id]["finished_at"] = _now_iso()
                _emit_siem_event(
                    "credential_check_failed",
                    "high",
                    {"check_id": check_id, "backend": backend, "error": str(exc)},
                )

        threading.Thread(target=_runner, name=f"hv-cred-check-{check_id}", daemon=True).start()
        return jsonify({"status": "started", "check_id": check_id, "backend": backend})

    @app.route("/api/credentials/status/<int:check_id>")
    @api_login_required
    def api_credentials_status(check_id):
        """Get credential check status."""
        with _credential_jobs_lock:
            job = _credential_jobs.get(check_id)
        if not job:
            return jsonify({"error": "Credential check not found"}), 404
        return jsonify(job)
