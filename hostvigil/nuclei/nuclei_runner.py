...
class NucleiRunner:
    ...
    def __init__(self, config: dict, db_path: str):
        ...
        self.binary = str(config.get("binary_path", "nuclei"))
        ...
        self._binary_path = self._resolve_binary_path(self.binary)
        if not self._binary_path:
            logger.warning("Nuclei binary '%s' not found in PATH — vulnerability scanning unavailable", self.binary)
        else:
            logger.info("Nuclei binary found at %s", self._binary_path)

    @staticmethod
    def _resolve_binary_path(binary: str) -> Optional[str]:
        """Resolve nuclei binary path from config/path/PATH lookup."""
        if os.path.isabs(binary) or os.path.sep in binary or (os.path.altsep and os.path.altsep in binary):
            return binary if os.path.isfile(binary) else None
        return shutil.which(binary)

    def _get_binary(self) -> str:
        """Return resolved nuclei binary path or raise RuntimeError."""
        self._binary_path = self._resolve_binary_path(self.binary)
        if not self._binary_path:
    ...
    def run_scan(self, targets: List[str] = None) -> List[Dict]:
        ...
        try:
            ...
            output = self._execute(command, targets=targets if targets else [])
    ...
    def is_nuclei_available(self) -> bool:
        ...
        except (RuntimeError, FileNotFoundError, subprocess.TimeoutExpired, OSError):
            return False
    ...
    def _execute(self, command: List[str], targets: List[str] = None) -> str:
        """Run nuclei subprocess and capture output.

        Args:
            command: List of command arguments.
            targets: Optional list of targets for timeout calculation.
        ...
        target_count = len(targets) if targets else 100
