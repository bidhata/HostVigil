...
class Config:
    ...
    def validate_runtime(self) -> list[str]:
        """Return runtime-safety warnings without changing execution behavior."""
        warnings = []
        hv = self.hostvigil

        scanner = hv.get("scanner", {})
        naabu = scanner.get("naabu", {})
        rate = naabu.get("rate", 5000)
        threads = naabu.get("threads", 50)
        if not isinstance(rate, int) or rate <= 0:
            warnings.append(f"scanner.naabu.rate should be a positive integer, got {rate!r}")
        elif rate > 50000:
            warnings.append(f"scanner.naabu.rate={rate} may be too aggressive for stealth-sensitive environments")
        if not isinstance(threads, int) or threads <= 0:
            warnings.append(f"scanner.naabu.threads should be a positive integer, got {threads!r}")

        scanner_ports = scanner.get("ports", {})
        for profile, ports in scanner_ports.items():
            if not isinstance(ports, list):
                warnings.append(f"scanner.ports.{profile} should be a list")
                continue
            for p in ports:
                if not isinstance(p, int) or p < 1 or p > 65535:
                    warnings.append(f"scanner.ports.{profile} contains invalid port: {p!r}")
                    break

        nuclei = hv.get("nuclei", {})
        for key in ("rate_limit", "concurrency", "bulk_size", "timeout", "retries"):
            value = nuclei.get(key)
            if value is not None and (not isinstance(value, int) or value < 0):
                warnings.append(f"nuclei.{key} should be a non-negative integer, got {value!r}")

        stealth = hv.get("stealth", {})
        scan_start = stealth.get("scan_window_start")
        scan_end = stealth.get("scan_window_end")
        if scan_start is not None and (not isinstance(scan_start, int) or not (0 <= scan_start <= 23)):
            warnings.append(f"stealth.scan_window_start should be an integer in 0..23, got {scan_start!r}")
        if scan_end is not None and (not isinstance(scan_end, int) or not (0 <= scan_end <= 23)):
            warnings.append(f"stealth.scan_window_end should be an integer in 0..23, got {scan_end!r}")

        dashboard = hv.get("dashboard", {})
        refresh = dashboard.get("refresh_interval")
        if refresh is not None and (not isinstance(refresh, int) or refresh <= 0):
            warnings.append(f"dashboard.refresh_interval should be a positive integer, got {refresh!r}")

        db_path = hv.get("database", {}).get("path")
        if not db_path or not isinstance(db_path, str):
            warnings.append("database.path should be a non-empty string")

        credential_spray = hv.get("credential_spray", {})
        backend = str(credential_spray.get("backend", "native")).strip().lower()
        if backend not in {"native", "spearspray"}:
            warnings.append(f"credential_spray.backend should be 'native' or 'spearspray', got {backend!r}")

        timeout_seconds = credential_spray.get("timeout_seconds", 1800)
        if not isinstance(timeout_seconds, int) or timeout_seconds <= 0:
            warnings.append(f"credential_spray.timeout_seconds should be a positive integer, got {timeout_seconds!r}")

        native = credential_spray.get("native", {})
        min_delay = native.get("min_delay")
        max_delay = native.get("max_delay")
        if min_delay is not None and (not isinstance(min_delay, (int, float)) or min_delay < 0):
            warnings.append(f"credential_spray.native.min_delay should be >= 0, got {min_delay!r}")
        if max_delay is not None and (not isinstance(max_delay, (int, float)) or max_delay < 0):
            warnings.append(f"credential_spray.native.max_delay should be >= 0, got {max_delay!r}")

        if backend == "spearspray":
            spearspray = credential_spray.get("spearspray", {})
            required = ("domain", "username", "password", "domain_controller")
            missing = [k for k in required if not str(spearspray.get(k, "")).strip()]
            if missing:
                warnings.append(f"credential_spray.spearspray missing required fields: {', '.join(missing)}")
