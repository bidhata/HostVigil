...
def get_syslog_formatter(hostname: str | None = None, pid: int | None = None) -> logging.Formatter:
    """Return HostVigil's syslog-style formatter with UTC timestamp."""
    resolved_hostname = hostname or socket.gethostname()
    resolved_pid = pid or os.getpid()
    formatter = logging.Formatter(
        fmt=f"%(asctime)sZ {resolved_hostname} hostvigil[{resolved_pid}] %(levelname)s %(name)s %(message)s",
    )
...
def setup_logging(
    ...
    pid = os.getpid()
    file_handler.setFormatter(get_syslog_formatter(hostname=hostname, pid=pid))
    logger.addHandler(file_handler)
    setup_audit_logging(log_dir=log_path)
...
def setup_audit_logging(log_dir: str | Path = "data/logs", enabled: bool | None = None) -> logging.Logger:
    """Configure optional JSON-lines audit logging."""
    if enabled is None:
        enabled = os.environ.get("HOSTVIGIL_AUDIT_LOG_JSON", "0") in {"1", "true", "TRUE", "yes", "YES"}

    audit_logger = logging.getLogger("hostvigil.audit")
...
def log_audit_event(event_type: str, **fields: Any) -> None:
    """Write a structured audit event if audit logging is enabled."""
    audit_logger = logging.getLogger("hostvigil.audit")
    if not audit_logger.handlers:
        return
    if all(isinstance(h, logging.NullHandler) for h in audit_logger.handlers):
        return
    payload = {
        "ts": now_iso(),
    ...
    try:
        ...
        # Never fail operational flow because of audit logging.
        pass
...
CREATE TABLE IF NOT EXISTS webhooks (
    ...
    dedup_seconds INTEGER DEFAULT 60,
    enabled INTEGER DEFAULT 1,
    created_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS schedule_policies (
    ...
    port_profile TEXT DEFAULT 'standard',
    nuclei_enabled INTEGER DEFAULT 1,
    notes TEXT,
    created_at TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS baseline_snapshot_items (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    snapshot_id INTEGER NOT NULL,
    item_type TEXT NOT NULL,
    item_key TEXT NOT NULL,
    item_payload TEXT,
    FOREIGN KEY (snapshot_id) REFERENCES baseline_snapshots(id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_schedule_policies_name ON schedule_policies(name);
CREATE INDEX IF NOT EXISTS idx_vuln_comments_vuln ON vulnerability_comments(vulnerability_id, created_at);
CREATE INDEX IF NOT EXISTS idx_baseline_items_snapshot ON baseline_snapshot_items(snapshot_id, item_type);
CREATE INDEX IF NOT EXISTS idx_siem_events_created ON siem_events(created_at, severity);
CREATE INDEX IF NOT EXISTS idx_alert_dedup_key ON alert_dedup(dedup_key);
...
def _migration_0008_dashboard_perf_indexes(conn: sqlite3.Connection) -> None:
    """Add performance indexes for high-volume dashboard and API queries."""
    ...
    if _table_exists(conn, "schedules"):
        conn.execute("CREATE INDEX IF NOT EXISTS idx_schedules_created_at ON schedules(created_at)")
    if _table_exists(conn, "profiles"):
        conn.execute("CREATE INDEX IF NOT EXISTS idx_profiles_created_at ON profiles(created_at)")
    if _table_exists(conn, "webhooks"):
        conn.execute("CREATE INDEX IF NOT EXISTS idx_webhooks_created_at ON webhooks(created_at)")
    if _table_exists(conn, "nuclei_rules"):
        conn.execute("CREATE INDEX IF NOT EXISTS idx_nuclei_rules_created_at ON nuclei_rules(created_at)")
    if _table_exists(conn, "scan_personas"):
        conn.execute("CREATE INDEX IF NOT EXISTS idx_scan_personas_created_at ON scan_personas(created_at)")
    if _table_exists(conn, "credential_results"):
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_credential_results_success_user_host ON "
            "credential_results(success, username, host_id)"
        )
    if _table_exists(conn, "honeytokens"):
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_honeytokens_confidence_detected ON honeytokens(confidence, detected_at)"
        )
    if _table_exists(conn, "mitre_mappings"):
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_mitre_mappings_tactic_technique ON mitre_mappings(tactic, technique_id)"
        )
    if _table_exists(conn, "risk_timeline"):
        conn.execute("CREATE INDEX IF NOT EXISTS idx_risk_timeline_recorded_at ON risk_timeline(recorded_at)")
    if _table_exists(conn, "host_tags"):
        conn.execute("CREATE INDEX IF NOT EXISTS idx_host_tags_tag_host ON host_tags(tag, host_id)")


def _migration_0009_feature_pack(conn: sqlite3.Connection) -> None:
    """Add RBAC, lifecycle, schedule policies, baseline snapshots, and SIEM tables."""
    if _table_exists(conn, "users") and not _column_exists(conn, "users", "role"):
    ...
    if _table_exists(conn, "api_keys") and not _column_exists(conn, "api_keys", "permissions"):
        conn.execute("ALTER TABLE api_keys ADD COLUMN permissions TEXT DEFAULT 'read'")
    ...
    if _table_exists(conn, "vulnerabilities"):
        for col_def in (
            ...
            col_name = col_def.split()[0]
            if not _column_exists(conn, "vulnerabilities", col_name):
                conn.execute(f"ALTER TABLE vulnerabilities ADD COLUMN {col_def}")
        conn.execute("UPDATE vulnerabilities SET updated_at = matched_at WHERE updated_at IS NULL OR updated_at = ''")
    ...
    if _table_exists(conn, "schedules"):
        for col_def in (
            ...
            col_name = col_def.split()[0]
            if not _column_exists(conn, "schedules", col_name):
                conn.execute(f"ALTER TABLE schedules ADD COLUMN {col_def}")
    ...
    if _table_exists(conn, "webhooks"):
        for col_def in (
            ...
            col_name = col_def.split()[0]
            if not _column_exists(conn, "webhooks", col_name):
                conn.execute(f"ALTER TABLE webhooks ADD COLUMN {col_def}")
        if _column_exists(conn, "webhooks", "enabled"):
            conn.execute("CREATE INDEX IF NOT EXISTS idx_webhooks_enabled ON webhooks(enabled, created_at)")
    ...
    """)
    conn.execute("CREATE INDEX IF NOT EXISTS idx_schedule_policies_name ON schedule_policies(name)")
    conn.execute(
        "CREATE INDEX IF NOT EXISTS idx_vuln_comments_vuln ON vulnerability_comments(vulnerability_id, created_at)"
    )
    conn.execute(
    ...
    conn.execute("CREATE INDEX IF NOT EXISTS idx_siem_events_created ON siem_events(created_at, severity)")
    conn.execute("CREATE INDEX IF NOT EXISTS idx_alert_dedup_key ON alert_dedup(dedup_key)")
...
_MIGRATIONS: list[tuple[str, str, MigrationFn]] = [
    ...
    (
        "0008",
        "dashboard and API performance indexes",
        _migration_0008_dashboard_perf_indexes,
    ),
    (
        "0009",
        "feature pack (rbac, lifecycle, policies, baseline, siem)",
        _migration_0009_feature_pack,
    ),
]
