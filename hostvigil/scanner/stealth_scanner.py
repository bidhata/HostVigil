...
class StealthScanner:
    ...
    def scan_hosts(self, hosts: List[str], port_profile: str = "standard") -> List[Dict]:
        ...
        safe_hosts = self._sanitize_hosts(hosts)
        if not safe_hosts:
            logger.warning("No valid host IPs provided to scan_hosts")
            return []

        ports = self._resolve_port_profile(port_profile)
        if not ports:
            logger.warning("No valid TCP ports resolved for profile %s", port_profile)
            return []

        targets: List[Tuple[str, int]] = [(host, port) for host in safe_hosts for port in ports]
    ...
    @staticmethod
    def _sanitize_hosts(hosts: List[str]) -> List[str]:
        """Filter a host list down to valid IP addresses."""
        safe_hosts: List[str] = []
        for host in hosts:
            try:
    ...
    @staticmethod
    def _resolve_port_profile(port_profile) -> List[int]:
        """Resolve/validate a TCP port profile to unique integer ports."""
        raw_ports = (
            port_profile
        ...
        for port in raw_ports:
            try:
                ...
                continue
            if 1 <= port_num <= 65535:
                safe_ports.append(port_num)
        return sorted(set(safe_ports))
...
def _scan_udp_method(self, hosts: List[str], port_profile: str = "standard") -> List[Dict]:
    ...
    safe_hosts = self._sanitize_hosts(hosts)
    if not safe_hosts:
        logger.warning("No valid host IPs provided to scan_udp")
        return []

    # Resolve port list
    if isinstance(port_profile, list):
        raw_ports = port_profile
    else:
        raw_ports = UDP_PORT_PROFILES.get(port_profile, UDP_PORT_PROFILES["standard"])
    ports = []
    for port in raw_ports:
        try:
            port_num = int(port)
    ...
    if not ports:
        logger.warning("No valid UDP ports resolved for profile %s", port_profile)
        return []
