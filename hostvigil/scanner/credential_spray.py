...
class StealthCredentialSpray:
    ...
    def spray_all(self, creds: Optional[List[Tuple]] = None, services: Optional[List[str]] = None) -> List[Dict]:
        """
        ...
        targets = self._get_eligible_targets(services=services)
        if not targets:
    ...
    def _get_eligible_targets(self, services: Optional[List[str]] = None) -> List[Dict]:
        """
        ...
        try:
            ...
            allowed_ports = set(SERVICE_PORTS.keys())
            if services:
                requested = {str(s).strip().lower() for s in services if str(s).strip()}
                if "all" not in requested:
                    service_ports = {port for port, name in SERVICE_PORTS.items() if name in requested}
                    if service_ports:
                        allowed_ports = service_ports
            sprayable_ports = sorted(allowed_ports)
