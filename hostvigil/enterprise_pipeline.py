        ...
        try:
            ...
            for host in hosts:
                try:
                    ...
                    continue
            if not safe_hosts:
                return {"total_open_ports": 0, "results": {}}
            ...
            if shutil.which("naabu"):
                ...
                tmp.close()
                try:
                    quick_ports = self.orchestrator.config.scanner.get("ports", {}).get(
                        "quick", [22, 80, 443, 445, 3389]
                    )
                    safe_ports = [str(int(p)) for p in quick_ports if isinstance(p, int) and 1 <= int(p) <= 65535]
                    ports = ",".join(sorted(set(safe_ports), key=int))
                    if not ports:
                        return {"total_open_ports": 0, "results": {}}
                    ...
                    for line in result.stdout.strip().split("\n"):
                        if ":" in line:
                            try:
                                ...
                                if 1 <= port_num <= 65535:
                                    port_map[ip.strip()].append(port_num)
                ...
                results = self.orchestrator.scanner.scan_hosts(safe_hosts, port_profile="quick")
