...
ALLOWED_WIPE_TABLES = frozenset({"anomalies", "vulnerabilities", "ports", "hosts"})
ALLOWED_PORT_IMPORT_REPLACE_TABLES = frozenset({"vulnerabilities", "ports"})
...
def _delete_all_from_table(conn: sqlite3.Connection, table_name: str, allowed_tables: frozenset[str]) -> None:
    """Delete all rows from an allowlisted table name."""
    if table_name not in allowed_tables:
        raise ValueError(f"Refusing to wipe non-allowlisted table: {table_name}")
...
class DataImporter:
    ...
    def _wipe_tables(self, conn: sqlite3.Connection):
        ...
        for table in ALLOWED_WIPE_TABLES:
            if _table_exists(conn, table):
                _delete_all_from_table(conn, table, ALLOWED_WIPE_TABLES)
    ...
    def import_ports_csv(self, input_path: str, mode: str = "merge") -> Dict:
        ...
        try:
            ...
            if mode == "replace":
                for table in ALLOWED_PORT_IMPORT_REPLACE_TABLES:
                    if _table_exists(conn, table):
                        _delete_all_from_table(conn, table, ALLOWED_PORT_IMPORT_REPLACE_TABLES)
