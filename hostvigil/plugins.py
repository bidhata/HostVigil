...
class PluginManager:
    ...
    def __init__(self, plugin_dir: str = "plugins", strict_contract: bool = False):
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if issubclass(attr, DiscoveryPlugin) and attr is not DiscoveryPlugin:
                    instance = attr()
                    if not self._validate_plugin_contract(instance, DiscoveryPlugin, "discover"):
                        continue
                    ...
                    instance = attr()
                    if not self._validate_plugin_contract(instance, ScannerPlugin, "scan"):
                        continue
                    ...
                    instance = attr()
                    if not self._validate_plugin_contract(instance, AnalysisPlugin, "analyze"):
                        continue
    ...
    def _validate_plugin_contract(self, instance: Any, base_cls: type, required_method: str) -> bool:
        """Validate plugin metadata and required overrides."""
        cls = instance.__class__
        plugin_name = getattr(instance, "name", cls.__name__)

        issues = []
        if not isinstance(getattr(instance, "name", None), str) or not instance.name.strip():
            issues.append("name must be a non-empty string")
        if not isinstance(getattr(instance, "description", ""), str):
            issues.append("description must be a string")

        plugin_api_version = str(getattr(instance, "plugin_api_version", PLUGIN_API_VERSION))
        if plugin_api_version != PLUGIN_API_VERSION:
            issues.append(f"plugin_api_version={plugin_api_version!r} expected {PLUGIN_API_VERSION!r}")

        caps = getattr(instance, "capabilities", ())
        if not isinstance(caps, (list, tuple)) or not all(isinstance(c, str) for c in caps):
            issues.append("capabilities must be a list/tuple[str]")

        method = getattr(cls, required_method, None)
        base_method = getattr(base_cls, required_method, None)
        if method is None or method is base_method:
            issues.append(f"must override '{required_method}()'")

        if issues:
            msg = f"Plugin '{plugin_name}' failed contract checks: " + "; ".join(issues)
    ...
    def get_all_plugins(self) -> Dict:
        ...
        return {
            "discovery": [
                {
                    ...
                    "api_version": getattr(p, "plugin_api_version", "unknown"),
                }
                for p in self.discovery_plugins
            ],
            "scanner": [
                {
                    "name": p.name,
            ...
            "analysis": [
                ...
                for p in self.analysis_plugins
            ],
            "total": len(self.discovery_plugins) + len(self.scanner_plugins) + len(self.analysis_plugins),
        }
