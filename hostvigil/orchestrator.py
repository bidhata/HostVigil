...
from hostvigil.utils import get_db_connection, init_database, log_audit_event, now_iso, setup_logging, wal_checkpoint
...
class HostVigilOrchestrator:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        runtime_warnings = self.config.validate_runtime()
        for w in runtime_warnings:
            logger.warning("Runtime: %s", w)
        log_audit_event(
            "orchestrator_initialized",
            db_path=str(Path(self.db_path).resolve()),
            scale_warnings=len(scale_warnings),
    ...
    def run_once(self) -> dict:
        ...
        log_audit_event("pipeline_run_started", mode="single")
        ...
        try:
            ...
            self._record_pipeline_phase(
                pipeline_results, "scan", {"ports_found": scan_results.get("ports_found", 0)}, "post-scan"
            )
            ...
            if self.config.scanner.get("udp_scan_enabled", True):
                ...
                self._record_pipeline_phase(
                    pipeline_results, "udp_scan", {"ports_found": udp_results.get("ports_found", 0)}, "post-udp-scan"
                )
            ...
            log_audit_event("pipeline_run_completed", success=True, phases=list(pipeline_results["phases"].keys()))
            ...
            log_audit_event("pipeline_run_completed", success=False, interrupted=True)
            ...
            log_audit_event("pipeline_run_completed", success=False, error=str(e))
    ...
    def _record_pipeline_phase(
        self, pipeline_results: dict, phase_name: str, phase_result: dict, post_delay: str = None
    ) -> None:
        """Record a phase result and enforce shutdown/delay behavior uniformly."""
        pipeline_results["phases"][phase_name] = phase_result
        log_audit_event("pipeline_phase_completed", phase=phase_name, **phase_result)
        if self._shutdown_event.is_set():
            raise InterruptedError("Shutdown requested")
        if post_delay:
            self._stealth_delay(post_delay)
    ...
    def _scan_batch_parallel(self, hosts: list, ports_str: str, naabu_available: bool) -> dict:
        ...
        """
        safe_hosts = self._sanitize_scan_hosts(hosts)
        safe_ports_str = self._sanitize_ports_csv(ports_str)
        if not safe_hosts or not safe_ports_str:
            return {}

        if naabu_available:
            return self._naabu_batch_scan(safe_hosts, safe_ports_str)
        else:
            return self._fallback_batch_scan(safe_hosts, safe_ports_str)

    @staticmethod
    def _sanitize_scan_hosts(hosts: list) -> list:
        """Keep only valid IP addresses for subprocess-driven scan paths."""
        safe_hosts = []
        for host in hosts:
            try:
                ipaddress.ip_address(str(host))
                safe_hosts.append(str(host))
            except ValueError:
                continue
        return safe_hosts

    @staticmethod
    def _sanitize_ports_csv(ports_str: str) -> str:
        """Normalize comma-separated ports to validated integers in 1..65535."""
        safe_ports = []
        for token in str(ports_str or "").split(","):
            token = token.strip()
            if not token:
                continue
            try:
                ...
                continue
            if 1 <= port <= 65535:
                safe_ports.append(str(port))
        return ",".join(sorted(set(safe_ports), key=int))
    ...
    def _naabu_batch_scan(self, hosts: list, ports_str: str) -> dict:
        ...
        try:
            ...
            return self._parse_naabu_output(proc.stdout)
    ...
    @staticmethod
    def _parse_naabu_output(stdout: str) -> dict:
        """Parse naabu stdout (ip:port per line) into {ip: [ports]}."""
        from collections import defaultdict
        ...
        for line in (stdout or "").splitlines():
            ...
            try:
                ...
                if 1 <= port_num <= 65535:
                    parsed[ip.strip()].append(port_num)
            except (ValueError, IndexError):
                continue
        return dict(parsed)
