...
def cmd_doctor(orchestrator: HostVigilOrchestrator, args: argparse.Namespace) -> int:
    ...
    if args.verbose:
        ...
        try:
            runtime_warnings = orchestrator.config.validate_runtime()
            if runtime_warnings:
                for w in runtime_warnings:
                    add("runtime_warning", False, w)
            else:
                add("runtime_check", True, "no runtime safety warnings detected")
        except Exception:
            pass
...
def main() -> int:
    ...
    if args.verbose:
        ...
        from hostvigil.utils import get_syslog_formatter

        stream_handler = logging.StreamHandler()
        stream_handler.setFormatter(get_syslog_formatter())
        logging.getLogger("hostvigil").addHandler(stream_handler)
